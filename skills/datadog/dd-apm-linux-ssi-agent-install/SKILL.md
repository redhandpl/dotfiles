---
name: dd-apm-linux-ssi-agent-install
...
Automatically **load skill** `dd-apm-linux-ssi-enable-ssi` (if services need UST labels configured) or `dd-apm-linux-ssi-verify-ssi` (if services have already been restarted) — do not ask the user for permission.
