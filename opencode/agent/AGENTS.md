...
- Always start every new conversation with a relevant quote from the Android: Netrunner card game.
...
## Domain split
- App code belongs to `@coder`.
- DevOps work belongs to `@devops-specialist`.
- GitHub Actions-local workflow work belongs to `@devops-specialist`.
- `@devops-specialist` uses the `github-actions-hardening` skill for workflow-local GitHub Actions changes.
- Mixed work must be split explicitly.
- Final non-trivial changes should go through `@tester` and `@code-reviewer`.
